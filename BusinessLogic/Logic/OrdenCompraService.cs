...
namespace BusinessLogic.Logic
{
    public class OrdenCompraService : IOrdenCompraService
    {
        ...
        private readonly IUnitOfWork _unitOfWork; //Con la instancia de la interfaz unit of work sustituye todas las llamadas a los repository y se crean de manera generica.
        ...
        {
            var carritoCompra = await _carritoCompraRepository.GetCarritoCompraAsync(carritoId); //1. Obtener el carrito de compras
            
            var items = new List<OrdenItem>();
            foreach (var item in carritoCompra.Items)
            {
                var productoItem = await _unitOfWork.Repository<Producto>().GetByIdAsync(item.Id); //Obtenemos el producto que esta dentro del item
        ...
        public async Task<IReadOnlyList<TipoEnvio>> GetTipoEnviosAsync()
        {
